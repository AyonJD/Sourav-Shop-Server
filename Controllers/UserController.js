...
module.exports.createUser = async (req, res) => {
    const { email, password, userName, role, isGoogleUser } = await req.body;
    ...
    try {
        ...
        if (user && isGoogleUserInitilizer) {
            ...
            res.status(200).json({ success: true, message: "Successfully created user", token: accessToken, result: { userName: userInitilizer, email:newInitilizer, role: roleInitilizer } });
        } else {
            const accessToken = jwt.sign(req.body, process.env.TOKEN, { expiresIn: "1d" });
